...
//This function is for placing an x or an o in square.
function placeXOrO(squareNumber) {

    //This condition ensure a sqare hasn`t been selected already.
    //This .some() method is used to check each element of selectedSqare array to
    ...
    if (!selectedSquares.some(element => element.includes(squareNumber))) {
        ...
        if (activePlayer === 'X') {
            ...
            select.style.backgroundImage = 'url("img/xN.png")';

            //Active player may only be X or O, so if not X, it must be O
        } else {
            //If activePlayer is equal to O, the o.png is placed in html.
            select.style.backgroundImage = 'url("img/oN.png")';
        }
    ...
    function computersTurn() {
        ...
        while(!success) {
            ...
            //If the random number evaluates returns true, the square has not been selected
...
//drawWinLine function is called to draw lione if condition is met.
function checkWinConditions() {
    ...
    //This function checks of an array includes 3 strings
    //It is used to check for each win condition
    function arrayIncludes(squareA, squareB, squareC) {

        //The next 3 var will be used to check for 3 in a row.
